"""
The tests for the vis_lint library
"""
...
def test_simple_line_chart():
    ...
    assert vis_lint(axes, fig, DEFAULT_CONFIGURATION) == [
...
def test_reading_level():
    ...
    assert vis_lint(axes, fig, reading_level_config) == []
...
def test_multi_series_plot_test():
    ...
    assert vis_lint(axes, fig, {"only-data-driven-visuals": 0.01}) == []
...
def test_custom_rule():
    ...
    assert vis_lint(axes, fig, custom_config) == [
        ("require-titles", 'Titles are required'),
        ("no-short-titles", 'Short titles are not allowed (must be greater than 1 word)'),
        ("require-axes", 'Axes must be labeled'),
        ("require-legend", 'A legend must be used'),
    ...
    assert vis_lint(axes, fig, custom_config) == [('require-legend', 'A legend must be used')]
